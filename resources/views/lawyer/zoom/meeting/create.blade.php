...
@section('title')
    <title>{{ __('Create Meeting') }}</title>
@endsection
@section('admin-content')
    <div class="main-content">
        <section class="section">
            <x-admin.breadcrumb title="{{ __('Create Meeting') }}" :list="[
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="form-group col-md-4">
                                            <x-admin.form-select name="client" id="client" class="select2"
                                                ...
                                                <option value="-1">{{ __('All Client') }}</option>
                                                @foreach ($users as $user)
                                                    <x-admin.select-option :selected="$user?->id == old('client',request('user'))" value="{{ $user?->id }}"
