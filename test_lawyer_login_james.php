...
/**
 * Test script for lawyer login - James Anderson
 * 
 * Usage: php test_lawyer_login_james.php
 * 
...
echo "=== Lawyer Login Test - James Anderson ===\n\n";
...
try {
    ...
    $request = Request::create('/lawyer/lawyer-login', 'POST', [
        'email' => $email,
        'password' => $password,
    ...
    if ($statusCheck && $emailVerified && $passwordValid) {
        // Try to login
        $loginSuccess = Auth::guard('lawyer')->attempt([
            'email' => $email,
            'password' => $password,
        ]);
        ...
        if ($loginSuccess) {
            ...
            echo "⚠ Login attempt failed (this might be due to guard configuration)\n";
            echo "But all credentials are correct, so manual login should work.\n";
        }
    } else {
