...
class MenuItem extends Model
{
    ...
    public function getAllAndLang($id,$code = 'en')
    {
        return $this->select('id','menu_id','link','parent_id','open_new_tab')->with(['translations' => function ($query) use ($code) {
            $query->where('lang_code', $code)->select('id','menu_item_id','label');
        }])->where("menu_id", $id)->orderBy("sort", "asc")->get();
    }
